...
const ProgressBar: React.FC<ProgressBarProps> = ({ current, total, trilhas, onTrilhaClick }) => {
  return (
    <div className="bg-white rounded-xl p-3 sm:p-6 shadow-lg">
      <h3 className="text-sm sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4">Progresso das Trilhas</h3>
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:flex sm:flex-wrap gap-2">
        {trilhas.map((trilha, index) => (
          <button
            key={trilha.id}
            onClick={() => onTrilhaClick(index)}
            className={`flex items-center justify-center sm:justify-start gap-1 sm:gap-2 px-2 sm:px-4 py-2 sm:py-3 rounded-lg font-medium transition-all duration-300 text-xs sm:text-sm min-h-[44px] sm:min-h-[40px] ${
              index === current
          ...
          >
            {index < current ? (
              <CheckCircle className="w-4 h-4 sm:w-4 sm:h-4 flex-shrink-0" />
            ) : (
              <Circle className="w-4 h-4 sm:w-4 sm:h-4 flex-shrink-0" />
            )}
            <span className="text-center sm:text-left leading-tight text-xs sm:text-sm font-medium">{trilha.title}</span>
          </button>
        ))}
      </div>
      <div className="mt-3 sm:mt-4">
        ...
        <div className="w-full bg-gray-200 rounded-full h-2 sm:h-2">
          <div 
            className={`h-2 sm:h-2 rounded-full transition-all duration-500 ${
